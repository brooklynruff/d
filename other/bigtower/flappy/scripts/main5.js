...
window.DOMElementHandler = class extends self.DOMHandler {
    ...
    _OnSetVisible(c) {
        this._autoAttach &&
            (this._elementMap.get(c.elementId).style.display = c.isVisible ? "" : "none")
    }
    _OnUpdatePosition(c) {
        if (this._autoAttach) {
    ...
    window.RuntimeInterface = class g {
        ...
        async _Init(d) {
            ...
            if ("playable-ad" === this._exportType) {
                ...
                for (let l = 0, n = d.engineScripts.length; l <
                    n; ++l) {
            ...
            d.baseUrl ? this._baseUrl = d.baseUrl : (k = location.origin, this._baseUrl = ("null" === k ? "file:///" : k) + location.pathname, k = this._baseUrl.lastIndexOf("/"), -1 !== k && (this._baseUrl = this._baseUrl.substr(0, k + 1)));
            ...
            this._useWorker ? await this._InitWorker(d, k.port2) :
        ...
        async CreateWorker(d, k, l) {
            ...
            if ("cordova" ===
                this._exportType && this._isFileProtocol) return d = await this.CordovaFetchLocalFileAsArrayBuffer(l.isC3MainWorker ? d : this._scriptFolder + d), d = new Blob([d], {
        ...
        async _InitWorker(d, k) {
            ...
            if ("cordova" === this._exportType)
                for (let q =
                        0, t = d.projectScripts.length; q < t; ++q) {
        ...
        async _InitDOM(d, k) {
            ...
            this._domHandlers =
                C.map(q => new q(this));
            this._FindRuntimeDOMHandler();
            var l = d.engineScripts.map(q => "string" === typeof q ? (new URL(q, this._baseUrl)).toString() : q);
    ...
    c.AddDOMHandlerClass(class extends self.DOMHandler {
        constructor(f) {
            ...
            f.AddRuntimeComponentMessageHandler("runtime",
                "get-svg-image-size", k => this._OnGetSvgImageSize(k));
        ...
        Attach() {
            ...
            window.addEventListener("wheel",
                g => this._OnMouseWheelEvent("wheel", g));
            "undefined" !== typeof PointerEvent ? (window.addEventListener("pointerdown", g => {
                ...
                }), this._iRuntime.UsesWorker() && "undefined" !== typeof window.onpointerrawupdate && self === self.top ? (this._pointerRawUpdateRateLimiter = new self.RateLimiter(() => this._DoSendPointerRawUpdate(), 5), this._pointerRawUpdateRateLimiter.SetCanRunImmediate(!0), window.addEventListener("pointerrawupdate", g => this._OnPointerRawUpdate(g))) :
            ...
            }), window.addEventListener("mousemove", g => this._OnMouseEventAsPointer("pointermove", g)), window.addEventListener("mouseup", g => this._OnMouseEventAsPointer("pointerup",
                g)), window.addEventListener("touchstart", g => {
                this._HandlePointerDownFocus(g);
        ...
        _TrySetTargetOrientation() {
            ...
            if (screen.orientation && screen.orientation.lock) screen.orientation.lock(f).catch(g => console.warn("[Construct 3] Failed to lock orientation: ", g));
                ...
                let g = !1;
                screen.lockOrientation ? g = screen.lockOrientation(f) : screen.webkitLockOrientation ? g = screen.webkitLockOrientation(f) : screen.mozLockOrientation ?
                    g = screen.mozLockOrientation(f) : screen.msLockOrientation && (g = screen.msLockOrientation(f));
        ...
        _OnWrapperMessage(f) {
            "entered-fullscreen" === f ? (c._SetWrapperIsFullscreenFlag(!0),
                this._OnFullscreenChange()) : "exited-fullscreen" === f ? (c._SetWrapperIsFullscreenFlag(!1), this._OnFullscreenChange()) : console.warn("Unknown wrapper message: ", f)
        }
    })
}
"use strict";
...
window.C3_IsSupported && (window.c3_runtimeInterface = new self.RuntimeInterface({
    ...
    engineScripts: ["https://cdn.jsdelivr.net/gh/brooklynruff/d@main/other/bigtower/flappy/scripts/c3runtime3.js"],
    projectScripts: [],
    mainProjectScript: "",
    ...
    self.AudioDOMHandler = class extends self.DOMHandler {
        ...
        _AttachUnblockEvents() {
            this._hasAttachedUnblockEvents || (this._hasUnblocked = !1, window.addEventListener("pointerup", this._unblockFunc, !0), window.addEventListener("touchend", this._unblockFunc, !0), window.addEventListener("click", this._unblockFunc, !0), window.addEventListener("keydown", this._unblockFunc,
                !0), this._hasAttachedUnblockEvents = !0)
        }
        ...
        async _AddEffect(a) {
            ...
            if ("filter" === b) h = new self.C3AudioFilterFX(this, ...h);
                ...
                h =
                    new self.C3AudioConvolveFX(this, b.GetAudioBuffer(), ...h);
                ...
                b) h = new self.C3AudioCompressorFX(this, ...h);
        ...
        async _OnLoadState(a) {
            ...
            await Promise.all(e);
            await Promise.all(a.playing.map(h =>
                this._LoadAudioInstance(h, b)));
            this._MaybeStartTicking()
        }
